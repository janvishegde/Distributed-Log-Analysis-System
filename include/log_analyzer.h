...
typedef struct { char ip[46]; int count; } IPEntry;
typedef struct { long error_count; long warn_count; long info_count; long lines_processed; IPEntry top_ips[MAX_IP_ENTRIES]; int ip_count; } ThreadResult;
typedef struct { long total_errors; long total_warnings; long total_info; long total_lines; IPEntry ip_table[MAX_IP_ENTRIES]; int ip_count; pthread_mutex_t lock; } GlobalResult;
typedef struct { Chunk chunk; ThreadResult result; GlobalResult *global; } WorkerArgs;
